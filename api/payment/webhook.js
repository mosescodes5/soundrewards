...
const txSchema = new mongoose.Schema({
  ...
  plisioId: String, orderId: String, description: String
}, { timestamps: true });
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (status === 'completed' || status === 'mismatch') {
      ...
      if (user) {
        user.activePlan = planId;  // ← use activePlan
        await user.save();
        await Transaction.create({
          userId: user._id, type: 'deposit',
          amount: parseFloat(source_amount || 0), currency,
          status: 'confirmed', plisioId: txn_id, orderId: order_number,
          description: `Plan upgrade to ${planId}`,
        });
      }
    }
