...
import {
  Music2, Home, Wallet, Users, Crown, Bell, Settings,
  LogOut, BarChart3, Star, Search, X, Menu, DollarSign, ArrowDownLeft,
} from "lucide-react";
import { C } from "../theme";
...
export function AppLayout({ page, onNavigate, children }) {
  const { t } = useLang();
  ...
  const notifs = [
    { icon: DollarSign, msg: "Ganaste $0.45 en Midnight Frequencies", t: "2m", color: C.success },
    { icon: Users,      msg: "Nuevo referido: user_hn",                t: "1h", color: C.teal },
    { icon: ArrowDownLeft, msg: "Retiro de $50 aprobado",             t: "3h", color: C.gold },
  ];
      ...
      <div style={{ display: "flex", flexDirection: "column", height: "100vh", background: C.bg, overflow: "hidden" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 16px", borderBottom: `1px solid ${C.border}`, background: C.surface, flexShrink: 0 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <button style={{ background: C.surface2, border: `1px solid ${C.border}`, borderRadius: 7, padding: 7, cursor: "pointer", color: C.muted, position: "relative" }} onClick={() => setNotifOpen(!notifOpen)}>
              <Bell size={15} />
          ...
          <div style={{ position: "fixed", inset: 0, zIndex: 300 }}>
            ...
            <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, background: C.surface, borderRadius: "16px 16px 0 0", padding: "20px 16px", zIndex: 10 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                <span style={{ fontWeight: 700, color: C.text }}>Menú</span>
              ...
              <button onClick={() => { onNavigate("landing"); setSidebarOpen(false); }} style={{ display: "flex", alignItems: "center", gap: 12, width: "100%", background: "transparent", border: "none", color: C.muted, padding: "12px 4px", cursor: "pointer", fontSize: 14, marginTop: 4 }}>
  ...
  return (
    <div style={{ display: "flex", height: "100vh", overflow: "hidden", background: C.bg }}>
      ...
      <div style={{ width: collapsed ? 60 : 220, flexShrink: 0, background: C.surface, borderRight: `1px solid ${C.border}`, display: "flex", flexDirection: "column", padding: "14px 10px", transition: "width 0.2s" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: collapsed ? "center" : "space-between", marginBottom: 22, padding: "0 4px" }}>
          {!collapsed ? (
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <div style={{ width: 28, height: 28, borderRadius: 7, background: C.goldDim, display: "flex", alignItems: "center", justifyContent: "center" }}><Music2 size={14} color={C.gold} /></div>
              <span style={{ fontWeight: 800, fontSize: 13, color: C.text }}>{t.brand}</span>
            </div>
          ) : (
            <div style={{ width: 28, height: 28, borderRadius: 7, background: C.goldDim, display: "flex", alignItems: "center", justifyContent: "center" }}><Music2 size={14} color={C.gold} /></div>
          )}
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 2 }}>
          {navItems.map(n => (
            <button key={n.page} onClick={() => onNavigate(n.page)} style={{ display: "flex", alignItems: "center", gap: 10, padding: "9px 12px", borderRadius: 9, border: "none", cursor: "pointer", background: page === n.page ? C.goldDim : "transparent", color: page === n.page ? C.gold : C.muted, fontWeight: page === n.page ? 600 : 400, fontSize: 13, width: "100%", justifyContent: collapsed ? "center" : "flex-start" }}>
        ...
        <div style={{ borderTop: `1px solid ${C.border}`, paddingTop: 10 }}>
          {!collapsed && (
            <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 12px", marginBottom: 4 }}>
              <div style={{ width: 28, height: 28, borderRadius: 7, background: C.goldDim, display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 800, fontSize: 11, color: C.gold }}>M</div>
              <div style={{ overflow: "hidden" }}>
                <p style={{ margin: 0, fontSize: 12, fontWeight: 600, color: C.text, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>mike_us</p>
                <Badge color={C.gold} bg={C.goldDim} small>Gold</Badge>
              </div>
            </div>
          )}
          <button onClick={() => onNavigate("landing")} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 12px", background: "transparent", border: "none", cursor: "pointer", color: C.muted, width: "100%", borderRadius: 8, fontSize: 12, justifyContent: collapsed ? "center" : "flex-start" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", minWidth: 0 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 24px", borderBottom: `1px solid ${C.border}`, background: C.surface, flexShrink: 0, gap: 12 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, flexShrink: 0 }}>
                  ...
                  {notifs.map((n, i) => (
                    <div key={i} style={{ display: "flex", gap: 10, padding: "11px 14px", borderBottom: `1px solid ${C.border}` }}>
                      ...
                      <div>
                        ...
                        <p style={{ margin: "2px 0 0", fontSize: 11, color: C.muted }}>{n.t}</p>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <button onClick={() => onNavigate("admin")} style={{ background: C.purpleDim, border: `1px solid ${C.purple}44`, color: C.purple, borderRadius: 8, padding: "6px 12px", cursor: "pointer", fontSize: 11, fontWeight: 700 }}>{t.nav.admin}</button>
          </div>
        </div>
